...
python -c "import sys;exit(0) if sys.version_info>=(3,9) else exit(1)" || (echo Python 3.9+ requerido & exit /b 1)

python -m venv vad_env
call "%~dp0vad_env\Scripts\activate.bat"
python -m pip install -U pip setuptools wheel

REM ---- TODAS las dependencias en ruedas binarias ----
pip install --no-cache-dir --only-binary=:all: ^
 numpy==1.26.4 soxr==0.3.7 soxr==0.3.7 transformers==4.54.1 silero-vad==5.1.2 webrtcvad-wheels==2.0.14 ^
